...
class RestController extends Controller {
  ...
  async index() {
    const { ctx } = this;
    const { query, aggregate } = ctx.request.query;
    ...
    try {
      ...
      if (!aggstr) {
        ...
        const ret = await model.find(q.find || {}).sort(q.sort || { _id: -1 }).skip(Number(q.skip))
          .limit(Math.min(q.limit, 100));

        const vv = ret.map(s => {
          return { id: s.id, info: s.info, createtime: s._id.getTimestamp() };
        });
        ctx.body = { data: vv, total };
      } else {
        if (!Array.isArray(q)) {
          ctx.status = 400;
          return;
